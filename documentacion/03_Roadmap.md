...
- [x] **T-2.2.4 · [Lógica] Transiciones de turno**
  ...
  **Decisiones:**
  1. **Columna nueva `shifts.closed_effects_applied_at`** (migración `fc1ed56c4a3d`, probada de ida y vuelta: los CHECK y el UNIQUE sobreviven): la marca de "los efectos del cierre ya se aplicaron".
  2. **"Una sola vez" lo decide la base, no el código:** `on_shift_closed` reclama la marca con un `UPDATE ... WHERE closed_effects_applied_at IS NULL` y solo quien logra modificar la fila corre los efectos. Aunque dos requests detecten el cierre a la vez (o uno tenga una vista vieja del turno), uno solo lo aplica. Se verificó que el test de esa carrera **falla si se quita la condición** del UPDATE.
  3. **Los efectos corren en la misma transacción que la marca:** si uno falla, se deshace todo y el próximo request lo reintenta (no queda un turno "cerrado a medias"). Hoy no hay efectos: cada fase registra el suyo con `register_close_effect` (Fase 4: congelar `product_stock`; Fase 11: marcar pedidos críticos). Además se guarda `status = CLOSED` para que la base cuente lo mismo que la marca.
  4. **`on_shift_closed` es seguro de llamar siempre** (no hace nada antes del cierre ni después de la primera vez); los endpoints de `T-2.4.x` lo llamarán al consultar el turno.
  5. **`to_production(shift, actor)` exige un ADMIN** (si no, `ForbiddenError`), **solo después del cierre** y **una sola vez** (`InvalidTransitionError`, 409). Antes de pasar a `IN_PRODUCTION` asegura que los efectos del cierre estén aplicados aunque nadie hubiera mirado el turno. Deja rastro en `audit_log` (`shift.to_production`, antes/después, quién, IP); un intento rechazado no audita. El paso masivo de pedidos `PAYMENT_APPROVED` → `IN_PREPARATION` se suma en la Fase 9, cuando existan los pedidos.
  _Prueba:_ tests: `current_status` antes/durante/después del cierre sin llamar nada más; `on_shift_closed` se aplica una sola vez aunque se detecte en dos requests seguidos; `to_production()` solo lo dispara el admin. _Depende de:_ T-2.2.2 **Verificado con 25 tests** (suite completa: 651 pasan): estado antes/durante/después del cierre sin llamar nada; los efectos corren al detectar el cierre (incluido el instante exacto) y **una sola vez** en dos requests seguidos y con vista vieja; efecto que falla → sin marca y reintento; `to_production` solo admin (customer y delivery → `ForbiddenError`), no antes del cierre, no dos veces, no desde `DISPATCHING`/`FINISHED`; auditoría con antes/después e IP; intento rechazado sin auditoría.
...
- [x] **T-2.4.1 · [Backend] `GET /api/v1/shift/current`**
  ...
  **Decisiones:**
  ...
  _Prueba:_ **Verificado con 15 tests de API** (suite completa: 666 pasan): abierto → `seconds_to_close > 0` (7200 exactos); cerrado → `CLOSED` y 0; antes de abrir → `SCHEDULED` y no se puede pedir; el instante exacto del cierre ya es `CLOSED`; forma exacta de la respuesta (con y sin turno); sin sesión; no-store; crea el turno una vez y no duplica; sábado → `NO_SERVICE` sin crear nada; "hoy" en fecha local; refleja la plantilla y la zona configuradas por el admin; turno en producción → `ordering_open: false`; los efectos del cierre se aplican una sola vez a través del endpoint. Además probado contra el servidor local con la hora real: un solo turno creado, cerrado y marcado una vez. _Depende de:_ T-2.2.2

- [x] **T-2.4.2 · [Backend] `GET /shift` y `PATCH /shift/{id}` y `POST /shift/{id}/transition` (admin)**
  Listar por fecha, ajustar horarios/ventana del turno, forzar `open`/`close`/`to_production`.
  **Decisiones:**
  1. **"Forzar abrir/cerrar" no cambia un estado, mueve el horario:** como abierto/cerrado se deriva de la hora y no se guarda (`T-2.2.2`), `open` pone la hora de apertura en el minuto actual y `close` la de cierre en el minuto actual (redondeado hacia abajo, así el turno ya queda en el estado pedido). Hay un solo modelo y la hora que muestra el turno es la real. `close` además aplica los efectos del cierre en el acto. Solo sobre el turno de hoy; `to_production` es el de `T-2.2.4`.
  2. **Transiciones sin sentido → 409 sin tocar nada:** abrir uno ya abierto, cerrar uno ya cerrado o que todavía no abrió, cerrarlo en el mismo minuto en que abrió, abrir uno ya cerrado (para eso hay que ampliar el cierre con el PATCH), cualquier acción sobre un turno ya en producción, o sobre un turno que no es el de hoy.
  3. **`PATCH` valida el turno resultante** con las mismas reglas y mensajes que la plantilla (`ShiftSchedule`, ahora compartido con `shift.default`): mover el cierre antes de la apertura o una ventana de cancelación más larga que el turno → 422 con el campo señalado; campos desconocidos → 422; `null` limpia solo las horas estimadas. Un PATCH que no cambia nada no audita.
  4. **Una vez aplicados los efectos del cierre, el turno queda bloqueado** en apertura/cierre/ventana de cancelación (409): reabrir no deshace lo que el cierre ya hizo (stock congelado, etc.). Las horas estimadas se pueden corregir siempre. Si nadie miró el turno desde que cerró (efectos sin aplicar), todavía se puede corregir el cierre.
  5. **`GET /shift` crea el turno de hoy si no existe** (abrir el panel, §9.9), lista del más reciente al más viejo, con `?date=` y paginación como los usuarios. Cada turno trae `status` de ciclo de vida completo (`OPEN`, `CLOSED`, `IN_PRODUCTION`…), `ordering_open` y los instantes en UTC.
  6. **Auditoría unificada** (`shift.update`, `shift.open`, `shift.close`, `shift.to_production`): `before`/`after` son objetos con solo los campos que cambiaron (el de `to_production` pasó de texto plano a `{"status": ...}`; todavía no había registros reales).
  _Prueba:_ **Verificado con 43 tests de API** (suite completa: 709 pasan): admin adelanta el cierre y `shift/current` lo refleja (`close_at`, `seconds_to_close` exactos); mover el cierre al pasado cierra el turno; PATCH parcial, `null`, ventana de cancelación; 8 casos inválidos sin efecto ni auditoría; campo desconocido; auditoría con solo lo cambiado (y sin cambio no audita); turno bloqueado tras el cierre pero no las estimadas; `open`/`close` ahora con auditoría y efectos aplicados una vez; 5 transiciones absurdas → 409; turno de otro día; `to_production` antes/después del cierre y sin más transiciones después; customer/delivery → 403, sin sesión → 401; id o acción desconocidos. _Depende de:_ T-2.2.4, T-1.5.2

- [x] **T-2.4.3 · [Frontend] `assets/js/countdown.js` + integración en el home**
  ...
  **Decisiones:**
  1. **Un solo reloj por página** (`getShiftClock()`), compartido por el chip del header y el ticket del home: una sola consulta a `/shift/current` alimenta a los dos. El chip se conecta con `bindHeaderChip()` en **toda página que use el header** (home y Mi cuenta), porque si no el chip de esas otras páginas seguiría mostrando el `01:42:15` inventado.
  2. **Corrige el desfase del celular:** guarda la diferencia entre la hora del servidor (`now`, tomada a mitad del viaje de ida y vuelta) y la del dispositivo, y cuenta contra los instantes `open_at`/`close_at` ya resueltos por el servidor. Un celular con el reloj mal puesto muestra igual el tiempo real.
  3. **Cambia de estado solo** al pasar la hora de apertura o de cierre, pero **quien manda es el servidor**: en cada cambio vuelve a preguntar (el admin pudo mover los horarios). También se re-sincroniza cada 5 minutos y al volver a la pestaña (un celular dormido frena los timers).
  4. **Cinco estados, ninguno inventado:** *Consultando* (mientras carga: texto neutro, nada que pueda ser falso), *Abierto* (cuenta hasta el cierre, "Cancelás hasta 11:40" que pasa a "Ya no se puede cancelar" al vencer la ventana), *Por abrir* (cuenta hasta la apertura), *Cerrado* (también si el admin ya arrancó la cocina, `ordering_open: false`), *Sin servicio* (día no operativo) y *Error* (servidor caído: reintenta cada 15 s y, si ya tenía datos, sigue con lo último que supo).
  5. **Frases con la hora real** (`spokenTime`): "hasta las 12 del mediodía", "desde las 8 de la mañana", "hasta las 9:13 de la noche". El HTML ya no trae horas escritas a mano; el badge, el titular y el ticket los completa `pages/home.js`.
  6. **Cambio de backend asociado:** `GET /shift/current` suma `cancel_deadline_time` (el límite de cancelación en hora local, `HH:MM`) para que el front no adivine la zona horaria del negocio (ahora configurable). Test actualizado.
  _Pendiente conocido:_ `format.js` sigue con la zona `America/Argentina/Buenos_Aires` escrita fija para `formatDate`/`formatTime`; hay que hacerla salir de la configuración cuando esas funciones se usen con horarios reales (Fase 6 en adelante).
  _Prueba:_ **Verificado en un navegador real (Chromium) contra el backend local**, escritorio y celular a la vez: turno abierto con el contador bajando segundo a segundo; a las 21:12:00 "Cancelás hasta…" pasó a "Ya no se puede cancelar"; a las 21:13:00 (la hora de cierre) **pasó a "Pedidos cerrados por hoy" en ambos sin recargar**; chip del header y de Mi cuenta; sin errores de JavaScript. Con respuestas del servidor simuladas: por abrir (y el paso a abierto solo, sin recargar), día sin servicio, turno en cocina, servidor caído con reintento automático a los 15 s, y un celular con 10 minutos de desfase (mostró los 90 s reales). _Depende de:_ T-2.4.1, T-0.4.4

- [x] **T-2.4.4 · [Frontend] `pages/admin/configuracion.html` (turnos)**
  ...
  **Decisiones:**
  1. **Dos cosas distintas, y la pantalla lo dice:** editar la *plantilla* (`shift.default`) solo afecta a los turnos que se creen desde ahora; el turno de hoy, si ya existe, es otro registro. Por eso el formulario trae una casilla **"Aplicar también al turno de hoy"** (marcada por defecto), que solo aparece si ese turno todavía se puede editar (efectos de cierre sin aplicar y no en cocina). Sin ella, el caso del roadmap ("el cambio impacta en `shift/current`") no se cumpliría el mismo día.
  2. **Un bloque "Turno de hoy" con el estado y los botones** *Abrir ahora*, *Cerrar ahora* y *Empezar a cocinar* (los endpoints de `T-2.4.2`, que no tenían pantalla). Cada botón solo está habilitado cuando tiene sentido (abrir si está por abrir, cerrar si está abierto, cocinar si ya cerró); cerrar y cocinar piden confirmación. Los errores del servidor se muestran tal cual.
  3. **Guardar hace hasta tres pasos en orden** y dice qué quedó hecho: plantilla → zona horaria → turno de hoy. Si un paso falla no se siguen los siguientes, y lo ya guardado se informa (por ejemplo: "la plantilla se guardó" junto al error de la zona horaria). Los errores 422 del backend salen debajo de cada campo (los nombres de la API de turnos se traducen a los del formulario) y los que no son de un campo, en un aviso general.
  3b. Como en el panel de usuarios, **se muestra lo que respondió el servidor** (el valor canónico guardado), sin volver a pedir la lista.
  4. **Navegación del panel:** pastillas *Usuarios · Configuración* en ambas pantallas (antes el panel no tenía forma de ir de una a otra). `api.js` suma `put`.
  5. Pastillas de los días escritas en el HTML y sin animación de color: con animación, al cargar el formulario se veía un destello de 0,15 s desde "desmarcado".
  _Prueba:_ **Verificado en un navegador real (Chromium) contra el backend local, con el admin de prueba:** carga la plantilla, la zona y el estado real de hoy; el admin cambia cierre, horas estimadas, ventana (20→45) y suma el sábado, guarda, y **`GET /shift/current` devuelve el cierre nuevo (23:30) y el límite de cancelación nuevo (22:45)**; validaciones visibles (cierre antes de abrir, ventana -5, sin días, zona inventada —con la plantilla ya guardada avisada—); *Cerrar ahora* (el cierre pasa al minuto actual y el home dice `CLOSED`) y *Empezar a cocinar* (queda en producción, todos los botones deshabilitados, casilla "aplicar a hoy" oculta); con el turno bloqueado, guardar la plantilla no intenta tocarlo; sin errores de JavaScript. También revisado en pantalla de celular. Configuración y turnos locales restaurados al terminar. _Depende de:_ T-2.4.2, T-2.1.3
...
- [x] **T-3.2.2 · [Backend] Modelos `Product`, `ProductImage` + repo + migración**
  §6.3. Repo: `list_public(category_id?, search?)`, `get`, `create`, `update`, `set_active`.
  **Decisiones:**
  1. **La base también protege el precio y la categoría:** `CHECK (base_price > 0)` y clave foránea a `categories` **sin** `ON DELETE CASCADE`, así que borrar una categoría con productos falla en vez de llevarse el catálogo puesto (las categorías se desactivan, no se borran). Índices por categoría y por `is_active`.
  2. **`list_public` muestra solo productos activos de categorías activas**, en el orden del menú (posición de la categoría → posición del producto → id), con sus imágenes y su categoría ya cargadas (evita una consulta por producto en `T-3.2.4`). Desactivar una categoría oculta sus productos sin tocarlos; reactivarla los devuelve, salvo los que estén desactivados por su cuenta.
  3. **Búsqueda sin acentos ni mayúsculas** (`search_key`, lógica pura): "sandwich" encuentra "Sándwich" y "noquis" a "Ñoquis". Busca en nombre y descripción, y **cada palabra** tiene que aparecer, en cualquier orden y aunque sea parcial (`"papas mila"`). Se filtra en Python y no en SQL a propósito —SQLite no ignora acentos y un catálogo de comida son decenas de productos—; si algún día fueran miles, es el punto a mover a la base (documentado en el código).
  4. **El repositorio aplica las reglas de `T-3.2.1` por sí mismo** (`create` y `update` validan todo, con todos los errores juntos): un producto no puede llegar a la base sin pasar por ellas. `update` solo cambia lo que se le pasa y, si algo es inválido, no cambia nada; `description=None` la borra.
  5. **Alta al final de su propia categoría** (`sort_order` cuenta por categoría). Se puede cargar un producto en una categoría inactiva.
  6. **`set_active` es la "baja lógica"**: un producto que ya tuvo pedidos no se borra, se oculta. `ProductImage` queda solo como modelo (relación con orden y borrado en cascada tanto por el ORM como por la base); la carga de archivos es `T-3.2.5`.
  _Prueba:_ **Verificado con 59 tests** (suite completa: 966 pasan): alta y lectura con normalización; precio inválido (0, negativo, decimal, texto, nulo) y categoría inexistente → error sin guardar nada, y varios errores a la vez; `list_public` sin inactivos, **sin productos de categorías inactivas**, reactivar la categoría, respeto del orden del menú y de la posición del producto, filtro por categoría (inactiva o inexistente → vacío); búsqueda (mayúsculas, acentos en los dos sentidos, descripción, todas las palabras, combinada con la categoría, sin mostrar nunca ocultos); edición parcial, mover de categoría, borrar descripción y 5 casos inválidos sin efecto; `set_active`; la base rechaza precio ≤ 0, categoría inexistente y borrar una categoría con productos; imágenes en orden, borrado en cascada (ORM y SQL directo) y carga de imágenes y categoría en el listado. Migración probada `upgrade`/`downgrade`/`upgrade` sobre la base local y sumada al test de esquema. _Depende de:_ T-3.2.1

- [x] **T-3.2.3 · [Backend] Endpoints de productos (admin)**
  `POST /catalog/products`, `PATCH /{id}`, `DELETE /{id}` (baja lógica).
  **Decisiones:**
  1. **Con auditoría** (lo que se había dejado pendiente en `T-3.1.3`): un cambio de precio es plata, y si un cliente discute un monto hay que poder ver quién lo cambió y cuándo. Acciones `product.create` (con el estado inicial), `product.update` (con **solo los campos que de verdad cambiaron**, antes y después) y `product.deactivate`. Lo que no cambia nada no deja rastro (mandar el mismo precio, un PATCH vacío, dar de baja dos veces), y una petición rechazada o prohibida tampoco. Vive en un servicio nuevo, `CatalogAdminService`, y un helper compartido `services/audit.py` (`record_audit`) para el código nuevo; los tres bloques de auditoría anteriores (usuarios, configuración, turnos) no se tocaron.
  2. **Los errores salen todos juntos y en español:** los cuerpos de `POST`/`PATCH` aceptan cualquier valor (`Any`) y validan `catalog_service` (`T-3.2.1`), no Pydantic por tipo, así un precio `"100"` o `10.5` da el mensaje del negocio y varios campos inválidos vuelven en un solo 422 con la lista completa. Los campos desconocidos (`id`, `sort_order`, `images`) sí se rechazan. `ProductRepository.create` ahora recibe parámetros opcionales (`object`) para que un campo faltante sea un error de validación y no un `TypeError`.
  3. **`DELETE` es baja lógica y responde 204**: el producto se oculta y **no se borra** (puede tener pedidos). Es idempotente (uno ya inactivo también da 204 sin auditar de nuevo). Volver a mostrarlo es `PATCH {"is_active": true}`.
  4. **`ProductOut` incluye `images`** (vacío hasta `T-3.2.5`) para que la forma de la respuesta no cambie cuando llegue la carga de imágenes. El precio va siempre en centavos.
  5. Id inexistente → 404 en `PATCH` y `DELETE`; cliente y repartidor → 403; sin sesión → 401.
  _Prueba:_ **Verificado con 51 tests de API** (suite completa: 1017 pasan): alta (forma exacta, visible en el catálogo público, inactivo y oculto, en categoría inactiva, campos normalizados); 6 precios inválidos (0, negativo, decimal, texto, nulo, booleano) → 422 sin crear ni auditar; categoría inexistente; **todos los errores juntos** (con nombre, categoría y precio mal) y cuerpo vacío; campos no permitidos; edición (parcial, mover de categoría, borrar descripción con `null`, 6 casos inválidos sin ningún cambio, id inexistente); **baja lógica** (sigue en la base, sale del catálogo público, idempotente, se reactiva con PATCH); cliente/repartidor → 403 y sin sesión → 401 en cada operación de escritura; auditoría (alta, cambio de precio con antes/después, solo campos realmente cambiados, sin cambio real no audita, baja una sola vez, nada al ser rechazado). _Depende de:_ T-3.2.2, T-1.5.2
